...
from .util import Conv1d_same_padding
from .load_data import load_data

class Net(torch.nn.Module): 
    def __init__(self, flag): 
        super(Net, self).__init__() 
        X_data, _, _ = load_data("E_O")
    ...
    def forward(self, x): 
        if self.flag == "E_O":
            label_0 = self.data[ 0,:].reshape(1,1,5).float()   # 1V
            label_1 = self.data[12,:].reshape(1,1,5).float()   # 1V
            label_2 = self.data[ 9,:].reshape(1,1,5).float()   # 1V
        else:
